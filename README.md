...
- [Second Opinion](https://second-opinion-production.up.railway.app) - Adversarial verification for AI agents: pay an independent skeptic to try to break a claim before you act on it. Three tiers — quick reasoning check ($0.03), single skeptic with live web research and citations ($0.20), three-skeptic panel with majority verdict ($0.75). Structured verdicts with calibrated confidence; async job API. USDC via x402 on Algorand, Base, and Solana (GoPlausible + Coinbase CDP facilitators). No accounts, no API keys. ([Discovery](https://second-opinion-production.up.railway.app/.well-known/x402) | [OpenAPI](https://second-opinion-production.up.railway.app/openapi.json) | [llms.txt](https://second-opinion-production.up.railway.app/llms.txt))
